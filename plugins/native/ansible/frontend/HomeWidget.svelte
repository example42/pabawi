<!--
  ...
  A lightweight summary tile displayed on the home page showing Ansible integration status.
  Uses Ansible's red color (#EE0000) for theming.

  Features:
  - Node count from inventory
  - Available playbooks count
  - Recent runs count
  - Health status indicator
  - Quick link to Ansible integration page

  Loads independently from /api/plugins/ansible/summary endpoint.

  @module plugins/native/ansible/frontend/HomeWidget
  @version 1.0.0
-->
<script lang="ts">
  ...
  let playbookCount = $derived(summary?.metrics.playbookCount ?? 0);
  let recentRuns = $derived(summary?.metrics.recentRuns ?? 0);
  ...
  async function loadData(): Promise<void> {
    ...
    try {
      ...
      summary = await api.get<PluginSummary>('/api/plugins/ansible/summary');
    } catch (err) {
      error = err instanceof Error ? err.message : 'Failed to load Ansible summary';
      console.error('Failed to load Ansible summary:', err);
    } finally {
      loading = false;
    }
  }
  ...
  function navigateToAnsible(): void {
    router.navigate('/integrations/ansible');
  }

  function executePlaybook(event: MouseEvent): void {
    event.stopPropagation();
    router.navigate('/integrations/ansible?tab=playbooks');
  }

  function viewInventory(event: MouseEvent): void {
    event.stopPropagation();
    router.navigate('/integrations/ansible?tab=inventory');
  }
</script>

<button
  type="button"
  onclick={navigateToAnsible}
  ...
  {:else if summary}
    ...
    <div class="flex items-center justify-between mb-3">
      <div class="flex items-center gap-2">
        ...
        <span class="font-semibold text-gray-900 dark:text-white">Ansible</span>
      </div>
    ...
    <div class="space-y-2">
      ...
      <div class="flex items-center justify-between">
        ...
        <span class="text-lg font-bold text-red-600 dark:text-red-400">{nodeCount}</span>
      </div>

      <!-- Playbook Count -->
      <div class="flex items-center justify-between">
        <span class="text-sm text-gray-600 dark:text-gray-400">Playbooks</span>
        <span class="text-lg font-bold text-gray-700 dark:text-gray-300">{playbookCount}</span>
      </div>

      <!-- Recent Runs -->
      {#if recentRuns > 0}
        <div class="flex items-center justify-between">
          <span class="text-sm text-gray-600 dark:text-gray-400">Recent Runs</span>
          <span class="text-lg font-bold text-gray-700 dark:text-gray-300">{recentRuns}</span>
        </div>
      {/if}
    ...
    <div class="mt-3 pt-3 border-t border-red-200/50 dark:border-red-800/50">
      <div class="flex gap-2">
        <button
          ...
          class="flex-1 px-2 py-1.5 text-xs font-medium text-red-700 dark:text-red-300 bg-red-100 dark:bg-red-900/30 hover:bg-red-200 dark:hover:bg-red-900/50 rounded transition-colors"
          title="Run Playbook"
        >
    ...
    <div class="mt-2 flex items-center justify-between">
      <span class="text-xs text-gray-500 dark:text-gray-400">Automation</span>
