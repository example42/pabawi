...
exports.PuppetDBPlugin = exports.PuppetDBPluginConfigSchema = void 0;
exports.createPuppetDBPlugin = createPuppetDBPlugin;
 ...
 */
const PqlQuerySchema = zod_1.z.object({
    query: zod_1.z.string().min(1).describe("PQL query string"),
 ...
 */
const NodesListSchema = zod_1.z.object({
    query: zod_1.z.string().optional().describe("Optional PQL filter query"),
...
const NodeFactsSchema = zod_1.z.object({
    node: zod_1.z.string().min(1).describe("Node certname to query facts for"),
    refresh: zod_1.z.boolean().optional().default(false).describe("Force refresh from source"),
});
/**
 * Schema for node reports query
 */
const NodeReportsSchema = zod_1.z.object({
    node: zod_1.z.string().min(1).describe("Node certname to get reports for"),
    limit: zod_1.z.number().optional().default(10).describe("Maximum number of reports"),
    offset: zod_1.z.number().optional().default(0).describe("Number of reports to skip"),
});
/**
 * Schema for specific report query
 */
const ReportSchema = zod_1.z.object({
    hash: zod_1.z.string().min(1).describe("Report hash identifier"),
});
/**
 * Schema for reports summary query
 */
const ReportsSummarySchema = zod_1.z.object({
    limit: zod_1.z.number().optional().default(100).describe("Maximum reports to analyze"),
    hours: zod_1.z.number().optional().describe("Hours to look back"),
});
/**
 * Schema for all reports query
 */
const AllReportsSchema = zod_1.z.object({
    limit: zod_1.z.number().optional().default(100).describe("Maximum number of reports"),
    offset: zod_1.z.number().optional().default(0).describe("Number of reports to skip"),
});
/**
 * Schema for node events query
 */
const NodeEventsSchema = zod_1.z.object({
    node: zod_1.z.string().min(1).describe("Node certname to get events for"),
    status: zod_1.z.enum(["success", "failure", "noop", "skipped"]).optional().describe("Filter by event status"),
    resourceType: zod_1.z.string().optional().describe("Filter by resource type"),
    startTime: zod_1.z.string().optional().describe("Start time (ISO 8601)"),
    endTime: zod_1.z.string().optional().describe("End time (ISO 8601)"),
    limit: zod_1.z.number().optional().describe("Maximum number of events"),
    reportHash: zod_1.z.string().optional().describe("Filter by report hash"),
});
/**
 ...
 */
const CatalogResourcesSchema = zod_1.z.object({
...
const ResourcesByTypeSchema = zod_1.z.object({
    resourceType: zod_1.z.string().min(1).describe("Resource type to query"),
    limit: zod_1.z.number().optional().default(100).describe("Maximum number of resources"),
    offset: zod_1.z.number().optional().default(0).describe("Number of resources to skip"),
});
/**
 * Schema for specific resource query
 */
const ResourceSchema = zod_1.z.object({
    resourceType: zod_1.z.string().min(1).describe("Resource type"),
    resourceTitle: zod_1.z.string().min(1).describe("Resource title"),
});
// =============================================================================
 ...
 */
exports.PuppetDBPluginConfigSchema = zod_1.z.object({
    serverUrl: zod_1.z.string().url().optional().describe("PuppetDB server URL"),
    port: zod_1.z.number().optional().describe("PuppetDB port"),
    ...
    circuitBreaker: zod_1.z.object({
        threshold: zod_1.z.number().optional().describe("Failure threshold before opening"),
        resetTimeout: zod_1.z.number().optional().describe("Time before attempting reset"),
        timeout: zod_1.z.number().optional().describe("Request timeout"),
...
class PuppetDBPlugin {
    ...
    // Configuration
    // =========================================================================
    configSchema = exports.PuppetDBPluginConfigSchema;
    defaultPermissions = {
    ...
    // Constructor
    // =========================================================================
    constructor(puppetDBService, logger, performanceMonitor) {
    ...
    createCapabilities() {
        return [
            ...
            // Standardized Facts Capabilities (Phase 1)
            {
                ...
                handler: async (params) => this.factsGet(params),
            ...
            },
            {
                ...
                handler: async (params) => this.factsRefresh(params),
                requiredPermissions: ["puppetdb.facts", "facts.read"],
                riskLevel: "read",
                schema: {
                    arguments: {
            ...
            // Standardized Reports Capabilities (Phase 1)
            {
                ...
                handler: async (params) => this.reportsList(params),
            ...
            },
            {
                ...
                handler: async (params) => this.reportsGet(params),
                requiredPermissions: ["puppetdb.reports"],
                riskLevel: "read",
                schema: {
                    arguments: {
            ...
            },
            {
                ...
                handler: async (params) => this.reportsQuery(params),
                requiredPermissions: ["puppetdb.reports"],
                riskLevel: "read",
                schema: {
                    arguments: {
            ...
            // Standardized Events Capabilities (Phase 1)
            {
                ...
                handler: async (params) => this.eventsList(params),
            ...
            },
            {
                ...
                handler: async (params) => {
                    ...
                    return this.eventsStream(params, callback);
                },
                requiredPermissions: ["puppetdb.events"],
                riskLevel: "read",
                schema: {
            ...
            },
            {
                ...
                handler: async (params) => this.eventsQuery(params),
                requiredPermissions: ["puppetdb.events"],
                riskLevel: "read",
                schema: {
                    arguments: {
     ...
     * Execute a PQL query
     */
    async executeQuery(params, context) {
     ...
     */
    async listNodes(params, context) {
     ...
     */
    async getNodeReports(params, context) {
        ...
        try {
            ...
            const reports = await this.puppetDBService.getNodeReports(validated.node, validated.limit, validated.offset);
     ...
     */
    async getReport(params, context) {
     ...
     */
    async getReportsSummary(params, context) {
        ...
        try {
            ...
            const summary = await this.puppetDBService.getReportsSummary(validated.limit, validated.hours);
     ...
     */
    async getAllReports(params, context) {
        ...
        try {
            ...
            const reports = await this.puppetDBService.getAllReports(validated.limit, validated.offset);
     ...
     */
    async getNodeEvents(params, context) {
     ...
     */
    async getCatalogResources(params, context) {
        ...
        try {
            ...
            const resources = await this.puppetDBService.getCatalogResources(validated.node, validated.resourceType);
     ...
     */
    async getSummaryStats(_params, context) {
     ...
     */
    async getResourceTypes(_params, context) {
     ...
     */
    async getResourcesByType(params, context) {
        ...
        try {
            ...
            const resources = await this.puppetDBService.getResourcesByType(validated.resourceType, validated.limit, validated.offset);
     ...
     */
    async getResource(params, context) {
        ...
        try {
            ...
            const resource = await this.puppetDBService.getResource(validated.resourceType, validated.resourceTitle);
            complete({
                resourceType: validated.resourceType,
                resourceTitle: validated.resourceTitle,
                found: !!resource
    ...
    async reportsList(params) {
        ...
        try {
            ...
            if (params.nodeId) {
                ...
                reports = await this.puppetDBService.getNodeReports(params.nodeId, params.limit || 100, params.offset || 0);
            }
            else {
                // Get all reports
                reports = await this.puppetDBService.getAllReports(params.limit || 100, params.offset || 0);
            }
            ...
            const standardReports = reports.map(r => ({
                id: r.hash,
                nodeId: r.certname,
                nodeName: r.certname,
    ...
    async reportsGet(params) {
        ...
        try {
            ...
            const standardReport = {
                ...
                resourceChanges: report.resource_events?.map((e) => ({
                ...
                logs: report.logs?.map((l) => l.message || String(l)),
                metadata: {
                    noop: report.noop,
    ...
    async reportsQuery(params) {
        ...
        try {
            ...
            let reports = await this.puppetDBService.getAllReports(params.limit || 100, params.offset || 0);
            // Apply filters
     ...
     */
    async eventsStream(params, _callback) {
    ...
    async eventsQuery(params) {
        ...
        try {
            ...
            const nodeId = params.filters.nodeId || params.filters.certname;
            if (!nodeId) {
...
exports.PuppetDBPlugin = PuppetDBPlugin;
// =============================================================================
// Plugin Factory
// =============================================================================
/**
 ...
 */
function createPuppetDBPlugin(puppetDBService, logger, performanceMonitor) {
    return new PuppetDBPlugin(puppetDBService, logger, performanceMonitor);
}
exports.default = PuppetDBPlugin;
//# sourceMappingURL=PuppetDBPlugin.js.map
