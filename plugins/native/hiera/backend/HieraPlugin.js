...
exports.HieraPlugin = exports.HieraPluginConfigSchema = void 0;
 ...
 */
const HieraLookupSchema = zod_1.z.object({
    node: zod_1.z.string().min(1).describe("Node certname to resolve key for"),
    key: zod_1.z.string().min(1).describe("Hiera key to lookup"),
    environment: zod_1.z.string().optional().describe("Puppet environment (default: production)"),
    lookupMethod: zod_1.z.enum(["first", "unique", "hash", "deep"]).optional().default("first")
        .describe("Merge strategy for lookup"),
});
/**
 * Schema for listing/searching Hiera keys
 */
const HieraKeysSchema = zod_1.z.object({
    query: zod_1.z.string().optional().describe("Search query (regex or glob pattern)"),
    limit: zod_1.z.number().optional().default(100).describe("Maximum number of keys to return"),
    offset: zod_1.z.number().optional().default(0).describe("Number of keys to skip"),
});
/**
 * Schema for getting key details
 */
const HieraKeyDetailSchema = zod_1.z.object({
    key: zod_1.z.string().min(1).describe("Hiera key name"),
});
/**
 * Schema for hierarchy info
 */
const HieraHierarchySchema = zod_1.z.object({
    node: zod_1.z.string().optional().describe("Node to resolve hierarchy paths for"),
    environment: zod_1.z.string().optional().describe("Puppet environment"),
});
/**
 * Schema for scanning hieradata
 */
const HieraScanSchema = zod_1.z.object({
    refresh: zod_1.z.boolean().optional().default(false).describe("Force rescan of hieradata"),
});
/**
 * Schema for node Hiera data
 */
const NodeHieraDataSchema = zod_1.z.object({
    node: zod_1.z.string().min(1).describe("Node certname"),
});
/**
 * Schema for key values across nodes
 */
const KeyValuesAcrossNodesSchema = zod_1.z.object({
    key: zod_1.z.string().min(1).describe("Hiera key to lookup across all nodes"),
});
/**
 * Schema for code analysis
 */
const CodeAnalysisSchema = zod_1.z.object({
    refresh: zod_1.z.boolean().optional().default(false).describe("Force re-analysis"),
    includeLint: zod_1.z.boolean().optional().default(true).describe("Include lint issues"),
    includeModuleUpdates: zod_1.z.boolean().optional().default(true).describe("Check for module updates"),
});
// =============================================================================
 ...
 */
exports.HieraPluginConfigSchema = zod_1.z.object({
    controlRepoPath: zod_1.z.string().describe("Path to Puppet control repository"),
    hieraConfigPath: zod_1.z.string().optional().default("hiera.yaml").describe("Path to hiera.yaml relative to control repo"),
    environments: zod_1.z.array(zod_1.z.string()).optional().default(["production"]).describe("Available Puppet environments"),
    factSources: zod_1.z.object({
        preferPuppetDB: zod_1.z.boolean().optional().default(true).describe("Prefer PuppetDB for facts"),
        localFactsPath: zod_1.z.string().optional().describe("Path to local facts directory"),
    }).optional().default({ preferPuppetDB: true }),
    cache: zod_1.z.object({
        enabled: zod_1.z.boolean().optional().default(true),
        ttl: zod_1.z.number().optional().default(300000).describe("Cache TTL in ms (default: 5 min)"),
        maxEntries: zod_1.z.number().optional().default(1000).describe("Maximum cache entries"),
...
class HieraPlugin {
    ...
    constructor(logger, performanceMonitor) {
        ...
        // Configuration
        // =========================================================================
        this.configSchema = exports.HieraPluginConfigSchema;
     ...
     */
    async lookupKey(params, context) {
        ...
        try {
            ...
            const result = await this.hieraService.resolveKey(validated.node, validated.key, validated.environment);
     ...
     */
    async getKeyDetails(params, context) {
     ...
     */
    async getHierarchy(params, context) {
     ...
     */
    async scanHieradata(params, context) {
     ...
     */
    async getNodeData(params, context) {
     ...
     */
    async getKeyValuesAcrossNodes(params, context) {
     ...
     * Run code analysis
     */
    async runCodeAnalysis(params, context) {
...
exports.HieraPlugin = HieraPlugin;
exports.default = HieraPlugin;
