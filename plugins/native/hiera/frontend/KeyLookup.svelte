<!--
  ...
  @module plugins/native/hiera/frontend/KeyLookup
  @version 1.0.0
-->
<script lang="ts">
  ...
  async function performLookup(): Promise<void> {
    ...
    try {
      ...
      const response = await api.get<LookupResult>(`/api/hiera/lookup?${params}`);
      result = response;
      ...
      <div class="flex-1 relative">
        ...
        {#if showSuggestions && recentKeys.length > 0}
          <div class="absolute z-10 top-full left-0 right-0 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg max-h-48 overflow-y-auto">
            <div class="px-3 py-1.5 text-xs text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700">Recent lookups</div>
            {#each recentKeys as recentKey (recentKey)}
              <button type="button" onclick={() => selectSuggestion(recentKey)} class="w-full px-3 py-2 text-left text-sm hover:bg-gray-50 dark:hover:bg-gray-700/50 text-gray-700 dark:text-gray-300">{recentKey}</button>
            {/each}
          </div>
        {/if}
      </div>
      <button type="button" onclick={performLookup} disabled={loading || !keyInput.trim()} class="px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 disabled:bg-gray-400 rounded-lg transition-colors">
        {#if loading}<LoadingSpinner size="xs" />{:else}Lookup{/if}
      </button>
    </div>
    {#if certname || environment !== 'production'}
      <div class="mt-1 flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
        {#if certname}<span>Node: <span class="text-gray-700 dark:text-gray-300">{certname}</span></span>{/if}
        {#if environment !== 'production'}<span>Env: <span class="text-gray-700 dark:text-gray-300">{environment}</span></span>{/if}
      </div>
    {/if}
  </div>

  {#if error}<ErrorAlert message={error} variant="inline" />{/if}
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              ...
              <span class="px-1.5 py-0.5 text-xs rounded bg-gray-200 dark:bg-gray-700 {getTypeColor(valueType)}">{valueType}</span>
              {#if result.interpolated}
                <span class="px-1.5 py-0.5 text-xs rounded bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300">interpolated</span>
              {/if}
            </div>
            <button type="button" onclick={copyValue} class="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-700 rounded" title="Copy value">
              {#if copied}
          ...
          {#if result.source}
            <div class="text-xs text-gray-500 dark:text-gray-400">Source: <span class="font-mono text-gray-600 dark:text-gray-300">{result.source}</span></div>
          {/if}
                ...
                {#each result.hierarchy as level, i (level.path)}
                  <div class="flex items-center gap-2 text-xs">
                    ...
                    {#if level.exists}
                      <svg class="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                    {:else}
                      <svg class="w-4 h-4 text-gray-300 dark:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                    {/if}
