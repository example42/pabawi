...
# Documentation route, configuration and schema contracts
node --test scripts/documentation/contracts.test.mjs
...
All infrastructure integrations (Bolt, PuppetDB, Puppetserver, Hiera, Ansible, SSH, AWS, Azure, Proxmox, Checkmk) are plugins declared in `backend/src/plugins/registry.ts`. Every plugin:
...
- **`services/`**: Cross-cutting services: `ExecutionQueue` (concurrent limiting, FIFO), `StreamingExecutionManager` (SSE real-time output), `AuthenticationService`, `BatchExecutionService`, `AuditLoggingService` (user-action audit log), `PuppetRunHistoryService` (persists Puppet run history), `ExpertModeService` (debug/verbose UI toggle), and RBAC services (`UserService`, `RoleService`, `PermissionService`, `GroupService`)
- **`validation/CommandWhitelistService.ts`**: command policy. **`services/journal/JournalService.ts`**: infrastructure event journal. **`integrations/NodeLinkingService.ts`**: cross-source identity correlation.
- **Console services**: `ConsoleSessionManager`, `ConsoleConnectionBroker` and `ConsoleWebSocketProxy` own capacity, session state, in-memory upstream material and relay termination. Provider adapters live under integrations; real Proxmox compatibility remains unverified.
- **`routes/`**: Express route factories. Factories receive explicit services and/or the container. Infrastructure mounts are shared in `mountInfrastructureRoutes.ts`. Async handlers use `routes/asyncHandler.ts`
...
Bolt command output is parsed from JSON; both `_output` and `_error` fields must be extracted from failed task results. Caching varies by integration and request scope; Checkmk monitoring data is live. See `docs/architecture.md` for the current data and execution boundaries.
...
Server and integration settings come from the process environment (source development loads `backend/.env`). Authentication policy is database-backed. Run `scripts/setup.sh` for interactive setup. Key variable groups: `PORT/HOST/LOG_LEVEL`, `JWT_SECRET` (required), `PABAWI_LIFECYCLE_TOKEN` (optional), `ENTRA_ID_*` (SSO), `BOLT_*`, `PUPPETDB_*`, `PUPPETSERVER_*`, `HIERA_*`, `ANSIBLE_*`, `SSH_*`, `AWS_*`, `AZURE_*`, `PROXMOX_*`, `CHECKMK_*`, `CONSOLE_*`, `COMMAND_WHITELIST*`, `CACHE_*`, `CONCURRENT_EXECUTION_LIMIT`, `MCP_ENABLED`.
...
- Service tests use in-memory SQLite; migration/recovery fixtures also use real WAL files and disposable PostgreSQL
