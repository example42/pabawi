...
describe('Auth Routes - POST /api/auth/login', () => {
  ...
  describe('Edge cases and error handling', () => {
    ...
    it('should handle database errors gracefully', async () => {
      ...
      // Database errors may return either 401 (masked) or 500 (unhandled)
...
describe('Auth Routes - POST /api/auth/logout', () => {
  ...
  describe('Successful logout (Requirements 1.6, 6.4)', () => {
    ...
    it('should revoke token after logout (Requirement 6.4)', async () => {
      ...
      expect(secondLogoutResponse.body.error.code).toBe('TOKEN_REVOKED');
      expect(secondLogoutResponse.body.error.message).toBe('Token has been revoked. Please login again.');
    });
  ...
  describe('Security considerations', () => {
    it('should not expose user information in error messages', async () => {
      ...
      expect(errorMessage).not.toContain('userId');
