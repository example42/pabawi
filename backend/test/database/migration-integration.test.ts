...
describe('Migration Integration Test', () => {
  ...
  it('should apply all migrations on initialization', async () => {
    ...
    // Should have applied all migrations (000 through 030, no 012 in source)
    expect(status.applied).toHaveLength(30);
    ...
    expect(status.applied.at(-1)?.id).toBe('030');
  ...
  it('should not re-apply migrations on subsequent initializations', async () => {
    ...
    // Should still have 30 applied, 0 pending
    expect(status.applied).toHaveLength(30);
    expect(status.pending).toHaveLength(0);
