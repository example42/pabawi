...
describe('Migration Integration Test', () => {
  ...
  it('should apply all migrations on initialization', async () => {
    ...
    // Should have applied all migrations (000 through 029, no 012 in source)
    expect(status.applied).toHaveLength(29);
    ...
    expect(status.applied[19].id).toBe('020');
    expect(status.applied[20].id).toBe('021');
    expect(status.applied[21].id).toBe('022');
    expect(status.applied.at(-1)?.id).toBe('029');
  ...
  it('should not re-apply migrations on subsequent initializations', async () => {
    ...
    // Should still have 29 applied, 0 pending
    expect(status.applied).toHaveLength(29);
    expect(status.pending).toHaveLength(0);
