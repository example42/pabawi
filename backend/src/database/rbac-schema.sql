-- RBAC Authorization System Database Schema
-- This schema implements Role-Based Access Control with users, groups, roles, and permissions
-- All IDs are UUIDs, timestamps are ISO 8601 format
...
-- Composite index for direct user-role-permission path lookup
CREATE INDEX IF NOT EXISTS idx_user_roles_composite ON user_roles(userId, roleId);

-- Composite index for user-group-role path lookup
CREATE INDEX IF NOT EXISTS idx_user_groups_composite ON user_groups(userId, groupId);
CREATE INDEX IF NOT EXISTS idx_group_roles_composite ON group_roles(groupId, roleId);
