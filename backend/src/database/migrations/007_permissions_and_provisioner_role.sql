...
INSERT INTO role_permissions (roleId, permissionId, assignedAt) VALUES
  ('role-provisioner-001', 'proxmox-read-001', datetime('now')),
  ('role-provisioner-001', 'proxmox-provision-001', datetime('now')),
  ('role-provisioner-001', 'proxmox-destroy-001', datetime('now')),
  ('role-provisioner-001', 'proxmox-lifecycle-001', datetime('now'));

-- Provisioner: read, provision, destroy, lifecycle for aws
INSERT INTO role_permissions (roleId, permissionId, assignedAt) VALUES
  ('role-provisioner-001', 'aws-read-001', datetime('now')),
  ('role-provisioner-001', 'aws-provision-001', datetime('now')),
  ('role-provisioner-001', 'aws-destroy-001', datetime('now')),
  ('role-provisioner-001', 'aws-lifecycle-001', datetime('now'));

-- Provisioner: read, note for journal
INSERT INTO role_permissions (roleId, permissionId, assignedAt) VALUES
  ('role-provisioner-001', 'journal-read-001', datetime('now')),
  ('role-provisioner-001', 'journal-note-001', datetime('now'));

-- Provisioner: read for integration_config
INSERT INTO role_permissions (roleId, permissionId, assignedAt) VALUES
  ('role-provisioner-001', 'integration_config-read-001', datetime('now'));
...
INSERT INTO role_permissions (roleId, permissionId, assignedAt) VALUES
  ('role-admin-001', 'proxmox-read-001', datetime('now')),
  ('role-admin-001', 'proxmox-lifecycle-001', datetime('now')),
  ('role-admin-001', 'proxmox-provision-001', datetime('now')),
  ('role-admin-001', 'proxmox-destroy-001', datetime('now')),
  ('role-admin-001', 'proxmox-admin-001', datetime('now'));

-- Administrator: all aws permissions
INSERT INTO role_permissions (roleId, permissionId, assignedAt) VALUES
  ('role-admin-001', 'aws-read-001', datetime('now')),
  ('role-admin-001', 'aws-lifecycle-001', datetime('now')),
  ('role-admin-001', 'aws-provision-001', datetime('now')),
  ('role-admin-001', 'aws-destroy-001', datetime('now')),
  ('role-admin-001', 'aws-admin-001', datetime('now'));

-- Administrator: all journal permissions
INSERT INTO role_permissions (roleId, permissionId, assignedAt) VALUES
  ('role-admin-001', 'journal-read-001', datetime('now')),
  ('role-admin-001', 'journal-note-001', datetime('now')),
  ('role-admin-001', 'journal-admin-001', datetime('now'));

-- Administrator: all integration_config permissions
INSERT INTO role_permissions (roleId, permissionId, assignedAt) VALUES
  ('role-admin-001', 'integration_config-read-001', datetime('now')),
  ('role-admin-001', 'integration_config-configure-001', datetime('now')),
  ('role-admin-001', 'integration_config-admin-001', datetime('now'));
