# Padawi v0.2.0 Features Guide

Version: 0.2.0

## Overview

Padawi version 0.2.0 introduces major enhancements that transform it from a Bolt-specific interface into a general-purpose remote execution platform. This guide covers all new features including PuppetDB integration, multi-source inventory, re-execution capabilities, and expert mode enhancements.
...
Padawi now supports multiple inventory sources, allowing you to view and manage nodes from both Bolt inventory files and PuppetDB. This provides a comprehensive view of your infrastructure from multiple perspectives.
...
PuppetDB integration provides deep visibility into your Puppet-managed infrastructure, including facts, reports, catalogs, and events. All PuppetDB data is accessible directly within Padawi's interface.
...
- 🟡 **Changed**: Resources were modified
...
2. **In Execution History:**
   - Re-executions marked with ↻ icon
...
│                                             │
│ Status: Success | Duration: 5.3s            │
│ Exit Code: 0                                │
│                                             │
│ [Copy Command] [Re-execute]                 │
└─────────────────────────────────────────────┘
```
...
│ ● nginx.service - nginx - high performance  │
│   web server                                │
│    Loaded: loaded (/usr/lib/systemd/system/ │
│      nginx.service; enabled; vendor preset: │
│      disabled)                              │
│    Active: active (running) since Mon       │
│      2024-01-15 10:00:00 UTC; 2h 30min ago  │
...
│                                             │
│ Jan 15 10:00:00 web-01 systemd[1]:          │
│   Starting nginx...                         │
│ Jan 15 10:00:00 web-01 systemd[1]:          │
│   Started nginx.                            │
│                                             │
│ [1,234 lines] [Scroll to top] [Search]      │
├─────────────────────────────────────────────┤
...
- Learning how Padawi constructs commands
...
- 🔵 **Bolt**: Data from Bolt inventory/execution
- 🟢 **PuppetDB**: Data from PuppetDB
...
```
┌─────────────────────────────────────────────┐
│ Facts (PuppetDB) ✓ Loaded                   │
├─────────────────────────────────────────────┤
│ [Facts display...]                          │
└─────────────────────────────────────────────┘

┌─────────────────────────────────────────────┐
│ Reports (PuppetDB) ⚠ Connection Failed      │
├─────────────────────────────────────────────┤
│ Unable to load reports from PuppetDB        │
│ [Retry]                                     │
└─────────────────────────────────────────────┘

┌─────────────────────────────────────────────┐
│ Execution History (Bolt) ✓ Loaded           │
├─────────────────────────────────────────────┤
...
│ ✓ Bolt                              │
│ Status: Connected                   │
│ Type: Execution Tool & Info Source  │
...
├─────────────────────────────────────────────┤
│ Total Nodes: 67 (Bolt: 25, PuppetDB: 42)   │
│ Recent Executions: 156                      │
│ Success Rate: 94.2%                         │
│ Active Integrations: 2 of 2                 │
└─────────────────────────────────────────────┘
```
...
Padawi v0.2.0 brings powerful new capabilities while maintaining full backward compatibility with v0.1.0. The multi-source architecture, PuppetDB integration, re-execution features, and expert mode enhancements provide a comprehensive platform for infrastructure management.
...
Happy automating with Padawi v0.2.0!
