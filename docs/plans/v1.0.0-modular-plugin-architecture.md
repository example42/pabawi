...
#### Step 4: Refactor Integration Manager
...
- Remove broken `instanceof` check (line 186)
- Replace type-based routing with capability-based routing

**New Architecture:**

- Use `CapabilityRegistry` for all operations
- Use `PluginLoader` instead of manual registration
- Simplify data aggregation (delegate to CapabilityRegistry)

**Updated Methods:**

- `async initializePlugins()`: Call `PluginLoader.loadAll()`, register capabilities/widgets
- `async executeCapability(user, capabilityName, params, debugContext)`: Route to CapabilityRegistry
- `getPluginMetadata(user?: User): PluginMetadata[]`: Return permission-filtered plugin list
- `async reloadPlugin(pluginName: string)`: Hot-reload support

**Remove:**

- `getExecutionTools()`
- `getInformationSources()`
- Type-specific aggregation methods
